...
Publish the read-only collector and fixtures in a progress PR, run focused tests and repository precheck, and record one current-PR read-only exercise at an exact revision. Rollback is removal of the adapter; the existing validator and protected `ci` requirement remain unchanged. #1434 later adopts the collector in the controller after its own review and gate evidence.
