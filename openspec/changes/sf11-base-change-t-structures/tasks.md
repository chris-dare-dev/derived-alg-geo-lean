...
- [ ] 5.1 Run `scripts/precheck.sh` with only the targeted Lean module selection for each changed chunk, plus the no-sorry, layering, style, and roadmap checks; verify that no full-repository build command is invoked locally. The retired trust-surface workflow is not a required check for this recut.
...
- [ ] 6.5 Run the required `ci` check and use the loop controller for the guarded PR handoff and closure of #1445; the retired trust-surface workflow is not part of this recut.
