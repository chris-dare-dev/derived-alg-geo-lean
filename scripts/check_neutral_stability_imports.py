...
only a direct public, non-meta import supplies the outer Stability export route.
...
class Header(NamedTuple):
    ...
    runtime_exported_imports: frozenset[str]
    ...
    for path, entry in zip(paths, results, strict=True):
        ...
        for item in result["imports"]:
            ...
            if item["isExported"]:
                exported.add(name)
                if not item["isMeta"]:
                    runtime_exported.add(name)
        headers[path] = Header(
            frozenset(all_imports), frozenset(exported), frozenset(runtime_exported)
        )
    return headers
...
def check_graph(headers: dict[Path, Header], root: Path) -> list[str]:
    ...
    failures: list[str] = []
    if GEOMETRIC_STABILITY not in by_name[OUTER].runtime_exported_imports:
        failures.append(
