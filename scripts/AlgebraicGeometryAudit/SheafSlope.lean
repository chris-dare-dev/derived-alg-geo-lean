/-
Mu-slope slice of the AlgebraicGeometry audit: the weak slope datum `Coh X` carries against a
polarization, the maximal destabilizing subobject, and the Harder-Narasimhan filtration built from
it (#903, #904, #905, #1121).

Everything recorded here is about the MU-SLOPE. None of it is a statement about Gieseker
stability, and `hasHNProperty` below is mu-Harder-Narasimhan existence, not Gieseker
Harder-Narasimhan existence. MO1.08 (#1319) moved these declarations out of the Gieseker
directory for exactly that reason, and this slice follows them. Split out so concurrent branches
append to different files; see the umbrella file for the contract and reading guide.
-/
...
`WithTop ℝ` honest slope are read off in both the positive-multiplicity and the zero regimes. -/
...
from `Abelian/Stability/Weak/Splitting.lean`. -/
