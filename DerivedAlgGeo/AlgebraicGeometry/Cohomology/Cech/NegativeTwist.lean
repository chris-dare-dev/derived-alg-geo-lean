...
`MvPolynomial.eq_zero_of_X_pow_dvd_of_isHomogeneous_of_lt` closes.

`GradedModule.intShiftPiece_eq_bot_of_neg` states the neighbouring fact — `intShift` is
`ℕ`-indexed, so a piece whose degree `n + d` is negative is trivial outright — and is deliberately
*not* on that path: the cross equation kills the numerator while its degree is still a natural
number. It is what makes the `ℕ`-indexing of `intShift` legible, and nothing below depends on it.
...
equation imported from `Algebra/MvPolynomial/DivMonomial`. That algebraic lemma is the
mathematical heart of the negative-twist vanishing; everything below is the geometric plumbing
that produces the cross equation from a Čech cocycle. -/
...
theorem num_eq_zero_of_intCechFace_eq_of_neg (d : ℤ) (hd : d < 0)
  ...
  · exact h0
  · exact MvPolynomial.num_eq_zero_of_cross_of_neg ι k hij d hd a b m _ _
      (by rw [hm, hdeg]) hmem hcross
