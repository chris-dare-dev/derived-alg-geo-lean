...
def picardDifference (L : Pic X.toScheme) (f : Pic X.toScheme → ℤ) :
    Pic X.toScheme → ℤ :=
  fun M ↦ f (M * L) - f M

/-- Successive finite differences in a list of Picard classes. -/
def picardMixedDifference :
    List (Pic X.toScheme) → (Pic X.toScheme → ℤ) → Pic X.toScheme → ℤ
  | [], f => f
...
def picardCoefficient (classes : List (Pic X.toScheme))
    (f : Pic X.toScheme → ℤ) : ℤ :=
  picardMixedDifference classes f 1

@[simp]
theorem picardMixedDifference_nil (f : Pic X.toScheme → ℤ) :
    picardMixedDifference [] f = f :=
  rfl

@[simp]
theorem picardMixedDifference_cons (L : Pic X.toScheme)
    (classes : List (Pic X.toScheme)) (f : Pic X.toScheme → ℤ) :
    picardMixedDifference (L :: classes) f =
      picardDifference L (picardMixedDifference classes f) :=
  rfl

theorem picardDifference_comm (L M : Pic X.toScheme) (f : Pic X.toScheme → ℤ) :
...
@[simp]
theorem picardDifference_one (f : Pic X.toScheme → ℤ) : picardDifference 1 f = 0 := by
  funext L
  simp [picardDifference]

/-- Splitting a tensor-product direction introduces one second-difference correction. -/
theorem picardDifference_mul_direction (L M : Pic X.toScheme)
    (f : Pic X.toScheme → ℤ) :
...
def PicardDegreeLE (d : ℕ) (f : Pic X.toScheme → ℤ) : Prop :=
  ∀ classes : List (Pic X.toScheme), classes.length = d + 1 →
    picardCoefficient classes f = 0

/-- Additivity of a top Picard coefficient in its first class. -/
theorem picardCoefficient_cons_mul (classes : List (Pic X.toScheme))
    (L M : Pic X.toScheme) (f : Pic X.toScheme → ℤ)
...
theorem picardCoefficient_middle_mul (before after : List (Pic X.toScheme))
    (L M : Pic X.toScheme) (f : Pic X.toScheme → ℤ)
...
def picardCoefficientAddHom (before after : List (Pic X.toScheme))
    (f : Pic X.toScheme → ℤ)
    (hf : PicardDegreeLE (before.length + after.length + 1) f) :
    Additive (Pic X.toScheme) →+ ℤ where
...
@[simp]
theorem picardCoefficientAddHom_apply (before after : List (Pic X.toScheme))
    (f : Pic X.toScheme → ℤ)
    (hf : PicardDegreeLE (before.length + after.length + 1) f)
    (L : Additive (Pic X.toScheme)) :
...
theorem picardCoefficient_middle_zpow (before after : List (Pic X.toScheme))
    (L : Pic X.toScheme) (m : ℤ) (f : Pic X.toScheme → ℤ)
    (hf : PicardDegreeLE (before.length + after.length + 1) f) :
    picardCoefficient (before ++ (L ^ m) :: after) f =
...
theorem mixedDifference_picardPolynomial {ι : Type*} [Fintype ι]
    (f : Pic X.toScheme → ℤ) (L : ι → Pic X.toScheme)
...
theorem topCoefficient_picardPolynomial {d : ℕ}
    (f : Pic X.toScheme → ℤ) (L : Fin d → Pic X.toScheme) :
...
structure TwistContext (D : FiniteCohomology X) (F : Coh X.toScheme) (d : ℕ) where
  eulerPic : Pic X.toScheme → ℤ
  twistFamily : ∀ (r : ℕ) (L : Fin r → Pic X.toScheme), CoherentTwistFamily F L
  geometricInduction : ∀ (r : ℕ) (L : Fin r → Pic X.toScheme),
    GeometricInduction D (twistFamily r L) d
  realization : ∀ (r : ℕ) (L : Fin r → Pic X.toScheme)
...
def structureSheafObject
    (h : Scheme.Modules.IsCoherent X.toScheme
      (SheafOfModules.unit X.toScheme.ringCatSheaf)) : Coh X.toScheme :=
  ⟨SheafOfModules.unit X.toScheme.ringCatSheaf, h⟩

/-- Snapper input specialized to twists of the structure sheaf. -/
structure IntersectionContext (D : FiniteCohomology X)
    (C : D.LinearConnectingSystem) (d : ℕ) where
  structureSheafCoherent : Scheme.Modules.IsCoherent X.toScheme
...
variable {D : FiniteCohomology X} {F : Coh X.toScheme} {d : ℕ}
...
variable {D : FiniteCohomology X} {C : D.LinearConnectingSystem} {d : ℕ}

/-- The intrinsic Euler characteristic of a Picard class. -/
def eulerPic (P : IntersectionContext D C d) : Pic X.toScheme → ℤ :=
  P.twists.eulerPic
...
theorem picardIntersectionNumber_congr (P : IntersectionContext D C d)
    {L M : Fin d → Pic X.toScheme} (h : ∀ i, L i = M i) :
...
theorem picardIntersectionNumber_comp_perm (P : IntersectionContext D C d)
    (L : Fin d → Pic X.toScheme) (σ : Equiv.Perm (Fin d)) :
...
theorem picardIntersectionList_middle_mul (P : IntersectionContext D C d)
    (before after : List (Pic X.toScheme)) (L M : Pic X.toScheme)
...
theorem picardIntersectionList_middle_zpow (P : IntersectionContext D C d)
    (before after : List (Pic X.toScheme)) (L : Pic X.toScheme) (m : ℤ)
    (hlength : before.length + after.length + 1 = d) :
    P.picardIntersectionList (before ++ (L ^ m) :: after) =
...
theorem picardIntersectionList_middle_one (P : IntersectionContext D C d)
    (before after : List (Pic X.toScheme)) :
    P.picardIntersectionList (before ++ (1 : Pic X.toScheme) :: after) = 0 := by
...
def cartierClassIntersectionNumber (P : IntersectionContext D C d)
    (classes : Fin d → CartierDivisor.ClassGroup X.toScheme) : ℤ :=
...
def cartierDivisorIntersectionNumber (P : IntersectionContext D C d)
    (divisors : Fin d → CartierDivisor X.toScheme) : ℤ :=
  P.cartierClassIntersectionNumber fun i ↦
    CartierDivisor.toClass X.toScheme (divisors i)
...
theorem cartierDivisorIntersectionNumber_eq_picard (P : IntersectionContext D C d)
    (divisors : Fin d → CartierDivisor X.toScheme) :
...
theorem cartierDivisorIntersectionNumber_eq_of_principalEquivalent
    (P : IntersectionContext D C d) {divisors divisors' : Fin d → CartierDivisor X.toScheme}
    (h : ∀ i, CartierDivisor.toClass X.toScheme (divisors i) =
      CartierDivisor.toClass X.toScheme (divisors' i)) :
    P.cartierDivisorIntersectionNumber divisors =
...
theorem cartierDivisorIntersectionNumber_add_principal
    (P : IntersectionContext D C d) (divisors : Fin d → CartierDivisor X.toScheme)
    (f : Fin d → Additive X.toScheme.functionFieldˣ) :
    P.cartierDivisorIntersectionNumber
        (fun i ↦ divisors i + CartierDivisor.principal X.toScheme (f i)) =
      P.cartierDivisorIntersectionNumber divisors := by
...
noncomputable def surfaceIntersectionPairing (P : IntersectionContext D C 2) :
    Additive (Pic X.toScheme) →ₗ[ℤ] Additive (Pic X.toScheme) →ₗ[ℤ] ℤ where
