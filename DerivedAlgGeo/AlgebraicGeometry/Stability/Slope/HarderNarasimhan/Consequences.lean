...
-/
import DerivedAlgGeo.AlgebraicGeometry.Stability.Slope.HarderNarasimhan.Existence
...
`Abelian/Stability/Weak/HarderNarasimhan.lean` or
`Abelian/Stability/Weak/Splitting.lean` to the filtration `Existence.lean` builds.

Everything here is about the **μ-slope**. The Gieseker order is not mentioned and not imported;
the one-step filtration of a Gieseker-semistable sheaf lives with the other comparison
statements, in `Stability/Comparison.lean`.
