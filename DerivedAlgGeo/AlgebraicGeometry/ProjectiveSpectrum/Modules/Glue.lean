...
/-!
# The section-extension lemma: a section over `D₊(f)` becomes a global section of a twist

`exists_globalSection_twistBy` is the deliverable of `#585`. For `F` quasi-coherent on `Proj 𝒜`
and `s` a section over `D₊(f)`, some `f ᵏ · s` is the restriction of a **global** section of a
twist of `F`.

## This is not Serre's theorem

It is **Hartshorne II.5.14(a)** / **Stacks 01PW**, the section-extension lemma.

Serre's theorem is Hartshorne **II.5.17** / EGA II 2.7.9: a coherent `F` on `Proj S` is a quotient
of a finite `⊕ O(-dᵢ)`, equivalently `F(n)` is generated by global sections for `n ≫ 0`. The
completed `#570` route passes through `#586`: `GlobalGeneration.lean` proves the global-generation
form `exists_epi_free_tensorTwist`, and `TwistInverse.lean` supplies the final untwisted coproduct
epimorphism. This file is the section-extension input to that route, not Serre's theorem by itself.

The distinction is stated here because it is the one a summary loses. `TwistSection.lean` calls
this "the extension theorem of `#585`", which is right; roadmap prose has more than once slid from
that to "Serre". This file alone does not prove Serre; the surjection is assembled downstream.
...
theorem exists_globalSection_twistBy_of_data (F : (Proj 𝒜).Modules)
  ...
  -- glue
  -- The discarded third component is uniqueness of `σ` for this local family; see the note on
  -- the theorem above for what recovering it would take.
...
## Two things this does not say, and what they would take

**The glued section is not claimed unique.** `TopCat.Sheaf.existsUnique_gluing'` returns `∃!`, and
the proof below discards the uniqueness half when it destructures. For the local family it is
built from, `σ` *is* unique; the statement simply does not carry that. A consumer needing
canonicity — the same `σ` reached by two routes, say — has to redo the gluing rather than recover
uniqueness from this. Strengthening the conclusion to `∃!` would mean naming the local family in
the statement, which is what the existential over `k` currently hides.

**The exponent cannot be raised here.** There is deliberately no `_of_le`: from a witness at `k`
one cannot conclude one at `k + j`. Raising would need `F(e k)(e j) ≅ F(e (k + j))` for arbitrary
`F`, and `TensorTwist.lean` records why the monoidal structure cannot supply it — `tensorAssocIso`
needs *both* outer factors invertible and `F` is not. `GlueUniform.lean` gets a uniform exponent
the only available way, by re-running this construction at a prescribed `n` rather than raising
what it produced. -/
