...
of a finite `⊕ O(-dᵢ)`, equivalently `F(n)` is generated by global sections for `n ≫ 0`. That is
`#570`, via `#586`, and it is proved in `GlobalGeneration.lean` as `exists_epi_free_tensorTwist`,
in the global-generation form. This file is one of its three steps.
