...
/-! ### Scheme-facing convenience namespace

As for K3 surfaces, "let `Y` be an Enriques surface" means
`(Y : Scheme) [Y.Over (Spec k)] [IsSmoothProperVariety k Y]
(C : CanonicalSheafData k Y 2) [IsEnriquesSurface k Y C]`. There is no bundled
surface carrier. The namespace below only gives Scheme-facing convenience theorems. -/
