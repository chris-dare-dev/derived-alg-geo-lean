...
the numerical half. Its source is a genuine a variety over `k` and coherent sheaves on its underlying
...
structure NumericalData (X : Scheme.{u}) [X.Over (Spec (CommRingCat.of k))] [IsVariety k X] (n : ℕ) (A : Type u) (N : Type v)
  ...
  coherentChernClasses : Coh X → ChernClassData A
  /-- Geometric Chern classes descend through the numerical class map. -/
  coherentChernClasses_classOf : ∀ F : Coh X,
  ...
  constructed. -/
  finiteCohomology : Cohomology.FiniteCohomology k X
  /-- The cohomological Euler characteristic descends through the numerical class map. -/
  coherentEulerCharacteristic_classOf : ∀ F : Coh X,
    chi (classOfHom (K₀Ab.of F)) = finiteCohomology.eulerCharacteristic F

namespace NumericalData

variable {X : Scheme.{u}} [X.Over (Spec (CommRingCat.of k))] [IsVariety k X] {n : ℕ} {A : Type u} {N : Type v}
...
structure SatisfiesHRR (D : NumericalData k X n A N) : Prop where
...
noncomputable abbrev coherentEulerCharacteristic (D : NumericalData k X n A N) :
    Coh X → ℤ :=
...
Chern classes. -/
@[reducible]
noncomputable def toNumericalVariety (D : NumericalData k X n A N) : NumericalVarietyData n A N where
  ring := D.ring
  rank := D.rank
...
theorem toNumericalVariety_satisfiesHRR (D : NumericalData k X n A N)
    (hD : SatisfiesHRR D) : D.toNumericalVariety.SatisfiesHRR :=
  ⟨hD.eq⟩

@[simp] theorem toNumericalVariety_rank (D : NumericalData k X n A N) :
    D.toNumericalVariety.rank = D.rank := rfl

@[simp] theorem toNumericalVariety_chComp (D : NumericalData k X n A N) (E : N) (i : ℕ) :
    D.toNumericalVariety.chComp E i =
      ChernClassData.chernCharacterComponent (D.chernClasses E) i := rfl

@[simp] theorem toNumericalVariety_toddComp (D : NumericalData k X n A N) (i : ℕ) :
...
theorem toNumericalVariety_chComp_two (D : NumericalData k X n A N) (E : N) :
...
theorem toNumericalVariety_chComp_three (D : NumericalData k X n A N) (E : N) :
...
theorem toNumericalVariety_chComp_four (D : NumericalData k X n A N) (E : N) :
...
theorem toNumericalVariety_toddComp_two (D : NumericalData k X n A N) :
...
theorem toNumericalVariety_toddComp_three (D : NumericalData k X n A N) :
...
theorem toNumericalVariety_toddComp_four (D : NumericalData k X n A N) :
...
@[simp] theorem toNumericalVariety_chi (D : NumericalData k X n A N) :
    D.toNumericalVariety.chi = D.chi := rfl

/-- The numerical class of a coherent sheaf. -/
noncomputable abbrev classOf (D : NumericalData k X n A N) (F : Coh X) : N :=
  D.classOfHom (K₀Ab.of F)

@[simp]
theorem classOf_apply (D : NumericalData k X n A N) (F : Coh X) :
    D.classOf F = D.classOfHom (K₀Ab.of F) := rfl

theorem classOfHom_of_iso (D : NumericalData k X n A N) {F G : Coh X} (e : F ≅ G) :
    D.classOfHom (K₀Ab.of F) = D.classOfHom (K₀Ab.of G) := by
  rw [K₀Ab.of_iso e]

theorem classOfHom_of_shortExact (D : NumericalData k X n A N)
...
theorem classOf_iso (D : NumericalData k X n A N) {F G : Coh X} (e : F ≅ G) :
    D.classOf F = D.classOf G := by
  rw [classOf, classOf, K₀Ab.of_iso e]

theorem classOf_shortExact (D : NumericalData k X n A N)
    (S : ShortComplex (Coh X)) (hS : S.ShortExact) :
...
classes. -/
theorem chernCharacter_classOf (D : NumericalData k X n A N) (F : Coh X) (i : ℕ) :
...
theorem chi_classOf (D : NumericalData k X n A N) (F : Coh X) :
...
theorem coherentChernCharacter_iso (D : NumericalData k X n A N) {F G : Coh X}
    (e : F ≅ G) (i : ℕ) :
...
theorem coherentChernCharacter_shortExact (D : NumericalData k X n A N)
    (S : ShortComplex (Coh X)) (hS : S.ShortExact) (i : ℕ) :
...
theorem coherentEulerCharacteristic_iso (D : NumericalData k X n A N) {F G : Coh X}
...
theorem coherentEulerCharacteristic_shortExact (D : NumericalData k X n A N)
    (S : ShortComplex (Coh X)) (hS : S.ShortExact) :
