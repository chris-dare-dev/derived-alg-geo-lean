...
/-- The perfect base-change generators are nonempty as soon as the source
component contains a zero object.

No preservation property of the external product is needed: evaluate it on
a zero object of the source component and a compact zero object of the base,
then use that value itself as a generator. -/
theorem perfectBaseChangeGenerators_nonempty
    (P : ObjectProperty (Dqc.SchemeQuasicoherentDerivedCategory X.left))
    [P.ContainsZero]
    (externalProduct : SourcePerfectPartCategory X P ⥤
      (CompactDqcFiber T ⥤
        Dqc.SchemeQuasicoherentDerivedCategory (X ⨯ T).left)) :
    (perfectBaseChangeGenerators X T P externalProduct).Nonempty := by
  obtain ⟨Z, hZ, hP⟩ := P.exists_prop_of_containsZero
  let F : SourcePerfectPartCategory X P :=
    ⟨Z, hP, ObjectProperty.isCompactObject_of_iso
      ((isZero_zero _).iso hZ) IsCompactObject.zero⟩
  let G : CompactDqcFiber T := ⟨0, IsCompactObject.zero⟩
  exact ObjectProperty.nonempty_of_prop
    ⟨F, G, ⟨Iso.refl ((externalProduct.obj F).obj G)⟩⟩
...
instance perfectBaseChangeEnvelope_isTriangulated
    ...
    (externalProduct : SourcePerfectPartCategory X P ⥤
      (CompactDqcFiber T ⥤
        Dqc.SchemeQuasicoherentDerivedCategory (X ⨯ T).left)) :
    (perfectBaseChangeEnvelope X T P externalProduct).IsTriangulated := by
  letI : (perfectBaseChangeGenerators X T P externalProduct).Nonempty :=
    perfectBaseChangeGenerators_nonempty X T P externalProduct
  dsimp [perfectBaseChangeEnvelope]
  infer_instance
...
/-- The quasicoherent base-change component generated by a source component
containing zero is triangulated. -/
instance quasicoherentBaseChangeComponent_isTriangulated
    (P : ObjectProperty (Dqc.SchemeQuasicoherentDerivedCategory X.left))
    [P.ContainsZero]
    (externalProduct : SourcePerfectPartCategory X P ⥤
      (CompactDqcFiber T ⥤
        Dqc.SchemeQuasicoherentDerivedCategory (X ⨯ T).left)) :
    (quasicoherentBaseChangeComponent X T P externalProduct).IsTriangulated := by
  dsimp [quasicoherentBaseChangeComponent]
  infer_instance
