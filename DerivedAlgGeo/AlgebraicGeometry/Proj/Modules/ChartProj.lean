...
coordinates: sections of `modulesSpecToSheaf.obj (chartRestrict …)` over opens of
...
`Scheme.Modules.restrictAppIso` is `Iso.refl`, so `Γ(chartRestrict 𝒜 F hg, U)` **is**
`Γ(F, degreeOneChart 𝒜 hg ''ᵁ U)` — nothing has to move. What is left is that the glue wants the
opens spelled `D₊(g)` and `D₊(g) ⊓ D₊(f)` rather than as chart images, and those are equal only
propositionally (`degreeOneChart_image_top`, `degreeOneChart_image_basicOpen`).
...
/-- **Extension across a degree-one chart, on `Proj 𝒜`.**
...
theorem exists_pow_smul_eq_res_image (F : (Proj 𝒜).Modules)
    ...
    {W V : (Proj 𝒜).Opens}
    (hW : degreeOneChart 𝒜 hg ''ᵁ ⊤ = W)
    (hV : degreeOneChart 𝒜 hg ''ᵁ
  ...
  subst hW
  subst hV
  obtain ⟨n, t, ht⟩ := exists_pow_smul_eq_res_chart 𝒜 F hf hg s
  exact ⟨n, t, ht.trans (isLocalizationElem_pow_smul_eq 𝒜 F hf hg Nat.one_pos n s)⟩
...
theorem exists_pow_smul_eq_res_image_uniform (F : (Proj 𝒜).Modules)
    ...
    {W V : ι → (Proj 𝒜).Opens}
    (hW : (fun i => degreeOneChart 𝒜 (hg i) ''ᵁ ⊤) = W)
    (hV : (fun i => degreeOneChart 𝒜 (hg i) ''ᵁ
  ...
  subst hW
  subst hV
  obtain ⟨n, hn⟩ := exists_pow_smul_eq_res_chart_uniform 𝒜 F hf hg s
  refine ⟨n, fun i => ?_⟩
  obtain ⟨t, ht⟩ := hn i
  exact ⟨t, ht.trans (isLocalizationElem_pow_smul_eq 𝒜 F hf (hg i) Nat.one_pos n (s i))⟩
