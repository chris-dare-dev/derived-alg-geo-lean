...
`DerivedAlgGeo.AlgebraicGeometry.Modules.Coherent.Abelian.Kernels`. Generic finite-presentation
...
specialization is installed by `DerivedAlgGeo.AlgebraicGeometry.Modules.Coherent.Abelian.Extensions`.
The module
`DerivedAlgGeo.AlgebraicGeometry.Modules.Coherent.Abelian.Basic` installs the abelian instance and
packages the inclusion as an exact functor.

The affine equivalence `Coh (Spec R) ≌ FGModuleCat R` is proved in
`DerivedAlgGeo.AlgebraicGeometry.Modules.Coherent.Affine.Comparison`.

Downstream developments should import
`DerivedAlgGeo.AlgebraicGeometry.Modules.Coherent.Abelian.Basic` for the completed stage-1 API.
