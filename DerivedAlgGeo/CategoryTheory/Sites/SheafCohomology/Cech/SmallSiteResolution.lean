...
`DerivedAlgGeo.CategoryTheory.Sites.SheafCohomology.Cech.Bicomplex` and
`DerivedAlgGeo.CategoryTheory.Sites.SheafCohomology.Cech.Comparison` thread an explicit
