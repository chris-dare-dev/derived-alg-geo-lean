...
This file develops choice-free mass for a bare slicing and additive charge.
...
`HNFiltration.classMass`, `Slicing.classMass`. The strong Bridgeland wrappers
live downstream in `StabilityCondition/Metric/Mass/Uniqueness.lean`; the weak
parent imports only this carrier-level theorem.
-/
