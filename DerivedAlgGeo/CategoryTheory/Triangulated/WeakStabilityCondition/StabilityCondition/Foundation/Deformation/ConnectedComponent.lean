...
theorem exists_basisNhd_subset_connectedComponent (σ : StabilityCondition.WithClassMap C v) :
  ...
  have hε8 : ε < 1 / 8 := by
    ...
    linarith
  refine ⟨ε, hε, hε8, ?_⟩
  exact basisNhd_subset_connectedComponent_small C σ hε₀ hε₀16 hWide hε hε_lt_ε₀
...
theorem stabilitySeminorm_dominated_of_connected (σ τ : StabilityCondition.WithClassMap C v)
    (h : ConnectedComponents.mk σ = ConnectedComponents.mk τ) :
    ∃ K : ENNReal, K ≠ ⊤ ∧
      ∀ (f : Λ →+ ℂ), stabilitySeminorm C σ f ≤ K * stabilitySeminorm C τ f := by
  let P : StabilityCondition.WithClassMap C v →
      StabilityCondition.WithClassMap C v → Prop :=
    fun ρ₁ ρ₂ ↦ ∃ K : ENNReal, K ≠ ⊤ ∧
      ∀ f : Λ →+ ℂ, stabilitySeminorm C ρ₁ f ≤ K * stabilitySeminorm C ρ₂ f
  ...
  have hlocal :
    ...
    intro x hx
    obtain ⟨ε₀, hε₀, hε₀16, hWide⟩ := x.exists_epsilon0_sixteenth C
    let δ : ℝ := ε₀ / 2
    have hδ : 0 < δ := by
      dsimp [δ]
      positivity
    ...
    intro y hy
    constructor
    · exact stabilitySeminorm_dominated_of_basisNhd C x y hδ hδ8 hy
    · exact stabilitySeminorm_center_dominates_of_basisNhd C x y hδ hδ8 hy
  have htrans :
    ...
    intro f
    calc
      stabilitySeminorm C x f ≤ K₁ * stabilitySeminorm C y f := hdom₁ f
      _ ≤ K₁ * (K₂ * stabilitySeminorm C z f) := by
        gcongr
        exact hdom₂ f
      _ = (K₁ * K₂) * stabilitySeminorm C z f := by rw [mul_assoc]
...
theorem finiteSeminormSubgroup_eq_of_connected (σ τ : StabilityCondition.WithClassMap C v)
  ...
  ext f
  show stabilitySeminorm C σ f < ⊤ ↔ stabilitySeminorm C τ f < ⊤
  obtain ⟨K₁, hK₁, hdom₁⟩ := stabilitySeminorm_dominated_of_connected C σ τ h
  obtain ⟨K₂, hK₂, hdom₂⟩ := stabilitySeminorm_dominated_of_connected C τ σ h.symm
  constructor
  · intro hf
...
surjective. The surjectivity ensures `classOf C v` generates all of `Λ`. -/
theorem eq_zero_of_vanishes_on_cl (U : Λ →+ ℂ)
    (hv : Function.Surjective v)
    (hU : ∀ E : C, U (classOf C v E) = 0) :
    U = 0 := by
...
Requires surjectivity of `v` so that vanishing on `classOf C v` implies vanishing on all of `Λ`. -/
theorem eq_zero_of_stabilitySeminorm_toReal_eq_zero (σ : StabilityCondition.WithClassMap C v)
    (hv : Function.Surjective v)
    {U : Λ →+ ℂ} (hfin : stabilitySeminorm C σ U < ⊤)
    (hzero : (stabilitySeminorm C σ U).toReal = 0) :
    U = 0 := by
  have hU_ne_top : stabilitySeminorm C σ U ≠ ⊤ := ne_top_of_lt hfin
  have hseminorm_zero : stabilitySeminorm C σ U = 0 := by
    rcases (ENNReal.toReal_eq_zero_iff _).mp hzero with h | h
    · exact h
    · exact (hU_ne_top h).elim
  have hvanish : ∀ E : C, U (classOf C v E) = 0 := by
    intro E
    by_cases hE : IsZero E
    · rw [classOf_isZero (C := C) (v := v) hE, map_zero]
    · obtain ⟨F, hn, _, _⟩ := σ.slicing.exists_hn_intrinsic_width C hE
      have hfactor :
          ∀ i : Fin F.n, U (classOf C v (F.toPostnikovTower.factor i)) = 0 := by
        intro i
        by_cases hi : IsZero (F.toPostnikovTower.factor i)
        · rw [classOf_isZero (C := C) (v := v) hi, map_zero]
        · have hbound :=
            stabilitySeminorm_bound_real C σ U hU_ne_top (F.semistable i) hi
          rw [hseminorm_zero, ENNReal.toReal_zero, zero_mul] at hbound
          exact norm_eq_zero.mp <| le_antisymm hbound (norm_nonneg _)
      rw [classOf_postnikovTower_eq_sum C v, map_sum]
      calc
        ∑ i : Fin F.n, U (classOf C v (F.toPostnikovTower.factor i))
          = ∑ i : Fin F.n, 0 := by
...
theorem Z_mem_finiteSeminormSubgroup (σ : StabilityCondition.WithClassMap C v) :
  ...
  show stabilitySeminorm C σ σ.Z < ⊤
  calc stabilitySeminorm C σ σ.Z
