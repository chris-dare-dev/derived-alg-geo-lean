...
Metric properties of `slicingDist`, seminorm properties of `stabilitySeminorm`,
the finite seminorm subgroup, and sector bound / comparison lemmas.
The definitions of `slicingDist` and `stabilitySeminorm` are in `StabilityCondition.Defs`.
-/
...
/-- A nearby semistable object has both intrinsic phases in the corresponding
open phase window. -/
theorem intrinsicPhaseBounds_of_semistable_slicingDist
    (s t : Slicing C) {E : C} {φ ε : ℝ} (hE : ¬IsZero E)
    (hP : t.P φ E) (hε : 0 < ε)
...
theorem norm_sum_exp_ge_cos_mul_sum {ι : Type*} {s : Finset ι}
    {m θ : ι → ℝ} (hm : ∀ i ∈ s, 0 ≤ m i) {α w : ℝ}
    (hw0 : 0 ≤ w) (hwπ : w < Real.pi)
    (hθ : ∀ i ∈ s, θ i ∈ Set.Icc α (α + w)) :
    Real.cos (w / 2) * ∑ i ∈ s, m i ≤
      ‖∑ i ∈ s, (m i : ℂ) * Complex.exp ((θ i : ℂ) * Complex.I)‖ := by
  let β := α + w / 2
...
def finiteSeminormSubgroup (σ : StabilityCondition.WithClassMap C v) : AddSubgroup (Λ →+ ℂ) where
  carrier := {U | stabilitySeminorm C σ U < ⊤}
  add_mem' {U V} hU hV := by
    change stabilitySeminorm C σ (U + V) < ⊤
    have hsub : stabilitySeminorm C σ (U + V) ≤
      ...
      calc ENNReal.ofReal (‖(U + V) (classOf C v E)‖ / ‖σ.charge E‖)
          ≤ ENNReal.ofReal (‖U (classOf C v E)‖ / ‖σ.charge E‖ +
              ‖V (classOf C v E)‖ / ‖σ.charge E‖) := by
            apply ENNReal.ofReal_le_ofReal
            rw [AddMonoidHom.add_apply, ← add_div]
            exact div_le_div_of_nonneg_right
              (norm_add_le _ _) (norm_nonneg _)
        _ = ENNReal.ofReal (‖U (classOf C v E)‖ / ‖σ.charge E‖) +
            ENNReal.ofReal (‖V (classOf C v E)‖ / ‖σ.charge E‖) :=
  ...
  zero_mem' := by
    change stabilitySeminorm C σ 0 < ⊤
    rw [stabilitySeminorm_zero]
    exact ENNReal.zero_lt_top
  neg_mem' {U} hU := by
    change stabilitySeminorm C σ (-U) < ⊤
    convert hU using 1
    simp [stabilitySeminorm, AddMonoidHom.neg_apply, norm_neg]
...
theorem sector_bound (σ : StabilityCondition.WithClassMap C v) (U : Λ →+ ℂ)
  ...
  have hUE : U (classOf C v E) = ∑ i : Fin F.n, U (classOf C v (P.factor i)) := by
    rw [classOf_postnikovTower_eq_sum C v P, map_sum]
  have hZE := σ.charge_postnikovTower_eq_sum P
  -- Seminorm bound on each factor
  have hMi : ∀ i : Fin F.n,
      ‖U (classOf C v (P.factor i))‖ ≤ M * ‖σ.charge (P.factor i)‖ := by
    intro i
    by_cases hi : IsZero (P.factor i)
    · simp [σ.charge_isZero hi, classOf_isZero (C := C) (v := v) hi]
  ...
  calc ‖U (classOf C v E)‖
      = ‖∑ i : Fin F.n, U (classOf C v (P.factor i))‖ := by rw [hUE]
    _ ≤ ∑ i : Fin F.n, ‖U (classOf C v (P.factor i))‖ := norm_sum_le _ _
...
theorem sector_bound' (σ : StabilityCondition.WithClassMap C v) (U : Λ →+ ℂ)
  ...
  have hwidth' : F.φ ⟨0, hn⟩ - F.φ ⟨F.n - 1, by omega⟩ ≤ η := by
    simp only [HNFiltration.phiPlus] at hP
    simp only [HNFiltration.phiMinus] at hM'
    rw [hP, hM']
    exact hwidth
  exact sector_bound C σ U F hn hη hη1 hwidth' hM0 hM
...
theorem norm_Z_le_of_tau_semistable (σ τ : StabilityCondition.WithClassMap C v)
    ...
    (hM_bound : ∀ (A : C) (ψ : ℝ), σ.slicing.P ψ A → ¬IsZero A →
      ‖(τ.Z - σ.Z) (classOf C v A)‖ ≤ M * ‖σ.charge A‖) :
    (1 - M / Real.cos (Real.pi * ε)) * ‖σ.charge E‖ ≤
      ‖τ.charge E‖ := by
  -- The σ-HN width of E is < 2ε (since E is τ-semistable of phase φ and d < ε)
  have hbounds := intrinsicPhaseBounds_of_semistable_slicingDist C σ.slicing τ.slicing hE hS hε hd
  ...
  --            ≥ ‖Z([E])‖ - M/cos(πε) * ‖Z([E])‖
  --            = (1 - M/cos(πε)) * ‖Z([E])‖
  have hkey : ‖(τ.Z - σ.Z) (classOf C v E)‖ ≤
  ...
  calc (1 - M / Real.cos (Real.pi * ε)) * ‖σ.charge E‖
    ...
    _ ≤ ‖σ.charge E‖ - ‖(τ.Z - σ.Z) (classOf C v E)‖ := by linarith
    _ ≤ ‖τ.charge E‖ := by
        have : ‖σ.charge E‖ ≤ ‖τ.charge E‖ +
          ‖(τ.Z - σ.Z) (classOf C v E)‖ := by
          calc ‖σ.charge E‖
              = ‖τ.charge E - (τ.Z - σ.Z) (classOf C v E)‖ := by
                congr 1; simp [AddMonoidHom.sub_apply]
            _ ≤ ‖τ.charge E‖ + ‖(τ.Z - σ.Z) (classOf C v E)‖ :=
                norm_sub_le _ _
        linarith
...
lemma stabilitySeminorm_bound_real (σ : StabilityCondition.WithClassMap C v) (U : Λ →+ ℂ)
    (hfin : stabilitySeminorm C σ U ≠ ⊤)
    {A : C} {ψ : ℝ} (hP : σ.slicing.P ψ A) (hA : ¬IsZero A) :
    ‖U (classOf C v A)‖ ≤ (stabilitySeminorm C σ U).toReal * ‖σ.charge A‖ := by
  ...
  have h1 : ENNReal.ofReal (‖U (classOf C v A)‖ / ‖σ.charge A‖) ≤
      stabilitySeminorm C σ U :=
    le_iSup_of_le A (le_iSup_of_le ψ (le_iSup_of_le hP (le_iSup_of_le hA le_rfl)))
  have hratio : ‖U (classOf C v A)‖ / ‖σ.charge A‖ ≤
      (stabilitySeminorm C σ U).toReal := by
    calc ‖U (classOf C v A)‖ / ‖σ.charge A‖
        = (ENNReal.ofReal (‖U (classOf C v A)‖ / ‖σ.charge A‖)).toReal :=
          (ENNReal.toReal_ofReal (div_nonneg (norm_nonneg _) (norm_nonneg _))).symm
      _ ≤ (stabilitySeminorm C σ U).toReal := ENNReal.toReal_mono hfin h1
  calc ‖U (classOf C v A)‖
      = ‖U (classOf C v A)‖ / ‖σ.charge A‖ * ‖σ.charge A‖ := by
        rw [div_mul_cancel₀ _ (ne_of_gt hZ_pos)]
    _ ≤ (stabilitySeminorm C σ U).toReal * ‖σ.charge A‖ :=
...
theorem stabilitySeminorm_lt_top_of_same_Z (σ τ : StabilityCondition.WithClassMap C v)
    (hZ : σ.Z = τ.Z)
    {ε : ℝ} (hε : 0 < ε) (hε1 : ε < 1 / 2)
    (hd : slicingDist C σ.slicing τ.slicing < ENNReal.ofReal ε)
    (U : Λ →+ ℂ) (hU : stabilitySeminorm C σ U < ⊤) :
    stabilitySeminorm C τ U < ⊤ := by
  have hU_ne : stabilitySeminorm C σ U ≠ ⊤ := ne_top_of_lt hU
  set M := (stabilitySeminorm C σ U).toReal with hM_def
  ...
  have hM_bound : ∀ (A : C) (ψ : ℝ), σ.slicing.P ψ A → ¬IsZero A →
      ‖U (classOf C v A)‖ ≤ M * ‖σ.charge A‖ :=
    fun A ψ hP hA ↦ stabilitySeminorm_bound_real C σ U hU_ne hP hA
  ...
  suffices h : stabilitySeminorm C τ U ≤
  ...
  have hbounds := intrinsicPhaseBounds_of_semistable_slicingDist C σ.slicing τ.slicing hE hP hε hd
  have hwidth : σ.slicing.phiPlus C E hE - σ.slicing.phiMinus C E hE ≤ 2 * ε := by
  ...
  calc ‖U (classOf C v E)‖ / ‖τ.charge E‖
...
theorem finiteSeminormSubgroup_eq_of_same_Z (σ τ : StabilityCondition.WithClassMap C v)
  ...
  constructor
  · exact stabilitySeminorm_lt_top_of_same_Z C σ τ hZ hε hε1 hd U
  · rw [slicingDist_symm] at hd
    exact stabilitySeminorm_lt_top_of_same_Z C τ σ hZ.symm hε hε1 hd U
...
`stabilitySeminorm_lt_top_of_near` and the reverse direction of **Proposition 6.3**. -/
theorem stabilitySeminorm_le_of_near (σ τ : StabilityCondition.WithClassMap C v)
    {ε : ℝ} (hε : 0 < ε) (hε1 : ε < 1 / 2)
    (hd : slicingDist C σ.slicing τ.slicing < ENNReal.ofReal ε)
    (hZdiff : stabilitySeminorm C σ (τ.Z - σ.Z) <
      ENNReal.ofReal (Real.cos (Real.pi * ε)))
    (U : Λ →+ ℂ) (hU : stabilitySeminorm C σ U ≠ ⊤) :
    stabilitySeminorm C τ U ≤
      ENNReal.ofReal ((stabilitySeminorm C σ U).toReal /
        (Real.cos (Real.pi * ε) - (stabilitySeminorm C σ (τ.Z - σ.Z)).toReal)) := by
  have hZdiff_ne : stabilitySeminorm C σ (τ.Z - σ.Z) ≠ ⊤ :=
    ne_top_of_lt (lt_trans hZdiff ENNReal.ofReal_lt_top)
  set M_U := (stabilitySeminorm C σ U).toReal
  set M_Z := (stabilitySeminorm C σ (τ.Z - σ.Z)).toReal
  ...
  have hMU_bound : ∀ (A : C) (ψ : ℝ), σ.slicing.P ψ A → ¬IsZero A →
      ‖U (classOf C v A)‖ ≤ M_U * ‖σ.charge A‖ :=
    fun A ψ hP hA ↦ stabilitySeminorm_bound_real C σ U hU hP hA
  have hMZ_bound : ∀ (A : C) (ψ : ℝ), σ.slicing.P ψ A → ¬IsZero A →
      ‖(τ.Z - σ.Z) (classOf C v A)‖ ≤ M_Z * ‖σ.charge A‖ :=
    fun A ψ hP hA ↦ stabilitySeminorm_bound_real C σ (τ.Z - σ.Z) hZdiff_ne hP hA
  apply iSup_le; intro E; apply iSup_le; intro φ
  apply iSup_le; intro hP; apply iSup_le; intro hE
  apply ENNReal.ofReal_le_ofReal
  obtain ⟨m, hm, hmZ⟩ := τ.compat φ E hP hE
...
theorem stabilitySeminorm_lt_top_of_near (σ τ : StabilityCondition.WithClassMap C v)
    {ε : ℝ} (hε : 0 < ε) (hε1 : ε < 1 / 2)
    (hd : slicingDist C σ.slicing τ.slicing < ENNReal.ofReal ε)
    (hZdiff : stabilitySeminorm C σ (τ.Z - σ.Z) <
      ENNReal.ofReal (Real.cos (Real.pi * ε)))
    ...
    stabilitySeminorm C τ U < ⊤ :=
  lt_of_le_of_lt
    (stabilitySeminorm_le_of_near C σ τ hε hε1 hd hZdiff U (ne_top_of_lt hU))
...
The forward direction uses `stabilitySeminorm_le_of_near`. The reverse direction uses
...
theorem finiteSeminormSubgroup_eq_of_basisNhd (σ τ : StabilityCondition.WithClassMap C v)
    ...
    (hd : slicingDist C σ.slicing τ.slicing < ENNReal.ofReal ε)
    (hZnorm : stabilitySeminorm C σ (τ.Z - σ.Z) <
  ...
  -- Forward: V(σ) ⊆ V(τ)
  have hfwd : ∀ U, stabilitySeminorm C σ U < ⊤ → stabilitySeminorm C τ U < ⊤ :=
    fun U hU ↦ stabilitySeminorm_lt_top_of_near C σ τ hε hε2 hd hZdiff U hU
  -- Reverse: V(τ) ⊆ V(σ)
  ...
  -- Step 5: Apply stabilitySeminorm_lt_top_of_near in reverse
  have hrev : ∀ U, stabilitySeminorm C τ U < ⊤ → stabilitySeminorm C σ U < ⊤ := by
    intro U hU
    have hd_sym : slicingDist C τ.slicing σ.slicing < ENNReal.ofReal ε := by
      rwa [slicingDist_symm]
    exact stabilitySeminorm_lt_top_of_near C τ σ hε hε2 hd_sym hZτ_bound U hU
  ext U; simp only [finiteSeminormSubgroup, AddSubgroup.mem_mk]
  exact ⟨hfwd U, hrev U⟩
