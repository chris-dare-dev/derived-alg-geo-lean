...
- **Lemma 6.2** (`stabilitySeminorm_dominated_of_connected`): seminorm equivalence on V(Σ).
...
theorem StabilityCondition.WithClassMap.exists_eq_Z_and_mem_basisNhd_of_stabilitySeminorm_lt_sin
    (σ : StabilityCondition.WithClassMap C v)
    (W : Λ →+ ℂ)
    (hW : stabilitySeminorm C σ (W - σ.Z) < ENNReal.ofReal 1)
    (ε₀ : ℝ) (hε₀ : 0 < ε₀)
    (hε₀16 : ε₀ < 1 / 16)
    (hWide : WideSectorFiniteLength C σ ε₀)
    (ε : ℝ) (hε : 0 < ε) (hεε₀ : ε < ε₀ / 2)
    (hsin : stabilitySeminorm C σ (W - σ.Z) < ENNReal.ofReal (Real.sin (Real.pi * ε))) :
    ∃ τ : StabilityCondition.WithClassMap C v, τ.Z = W ∧ τ ∈ basisNhd C σ ε := by
  let d := (stabilitySeminorm C σ (W - σ.Z)).toReal
  have hfinite : stabilitySeminorm C σ (W - σ.Z) ≠ ⊤ :=
    ne_top_of_lt (hW.trans ENNReal.ofReal_lt_top)
  have hd0 : 0 ≤ d := ENNReal.toReal_nonneg
  have hd_eq : ENNReal.ofReal d = stabilitySeminorm C σ (W - σ.Z) :=
    ENNReal.ofReal_toReal hfinite
  have hsin_pos : 0 < Real.sin (Real.pi * ε) :=
    Real.sin_pos_of_pos_of_lt_pi (by positivity) (by nlinarith [Real.pi_pos, hε₀16])
  have hd_sin : d < Real.sin (Real.pi * ε) := by
    rwa [← hd_eq, ENNReal.ofReal_lt_ofReal_iff hsin_pos] at hsin
  have hsin_le_one : Real.sin (Real.pi * ε) ≤ 1 := Real.sin_le_one _
  have hd1 : d < 1 := lt_of_lt_of_le hd_sin hsin_le_one
  have hd_mem : d ∈ Set.Icc (-1 : ℝ) 1 := ⟨by linarith, hd1.le⟩
  have hπε_mem : Real.pi * ε ∈ Set.Icc (-(Real.pi / 2)) (Real.pi / 2) := by
    constructor <;> nlinarith [Real.pi_pos, hε₀16]
  have hasin_lt : Real.arcsin d < Real.pi * ε := by
    have hmono := Real.strictMonoOn_arcsin hd_mem (Real.sin_mem_Icc _) hd_sin
    rwa [Real.arcsin_sin hπε_mem.1 hπε_mem.2] at hmono
  let δ := (Real.arcsin d / Real.pi + ε) / 2
  have hasin_nonneg : 0 ≤ Real.arcsin d := Real.arcsin_nonneg.mpr hd0
  have hδ : 0 < δ := by
    dsimp [δ]
    have : 0 ≤ Real.arcsin d / Real.pi := div_nonneg hasin_nonneg Real.pi_pos.le
    linarith
  have hδ_lt_ε : δ < ε := by
    dsimp [δ]
    have : Real.arcsin d / Real.pi < ε :=
      (div_lt_iff₀ Real.pi_pos).2 (by simpa [mul_comm] using hasin_lt)
    linarith
  have hδ_half : δ < ε₀ / 2 := hδ_lt_ε.trans hεε₀
  have hδ2 : δ ≤ 1 / 2 := by linarith [hδ_half, hε₀16]
  have hδ8 : δ < 1 / 8 := by linarith [hδ_half, hε₀16]
  have hd_sinδ : d < Real.sin (Real.pi * δ) := by
    have harg_lt : Real.arcsin d < Real.pi * δ := by
      dsimp [δ]
      have hdiv : Real.arcsin d / Real.pi < ε :=
        (div_lt_iff₀ Real.pi_pos).2 (by simpa [mul_comm] using hasin_lt)
      field_simp [Real.pi_ne_zero]
      nlinarith [Real.pi_pos]
    have hasin_mem : Real.arcsin d ∈ Set.Icc (-(Real.pi / 2)) (Real.pi / 2) :=
      ⟨(Real.neg_pi_div_two_le_arcsin d), Real.arcsin_le_pi_div_two d⟩
    have hδ_mem : Real.pi * δ ∈ Set.Icc (-(Real.pi / 2)) (Real.pi / 2) := by
      constructor <;> nlinarith [Real.pi_pos, hδ8]
    have hmono := Real.strictMonoOn_sin hasin_mem hδ_mem harg_lt
    rwa [Real.sin_arcsin (by linarith) hd1.le] at hmono
  have hsinδ : stabilitySeminorm C σ (W - σ.Z) <
      ENNReal.ofReal (Real.sin (Real.pi * δ)) := by
    rw [← hd_eq]
    exact (ENNReal.ofReal_lt_ofReal_iff
      (Real.sin_pos_of_pos_of_lt_pi (by positivity)
        (by nlinarith [Real.pi_pos, hδ8]))).2 hd_sinδ
  exact σ.exists_with_Z_mem_basisNhd C W hd0 hd1 (by rw [← hd_eq])
    hε₀ hε₀16 hWide hδ hδ_half hδ2 hδ8 hδ_lt_ε
    (by linarith [hεε₀, hε₀16]) hsinδ
...
theorem wideSectorFiniteLength_mono (σ : StabilityCondition.WithClassMap C v)
    ...
    WideSectorFiniteLength C σ ε₁ := by
  ...
  have hIncl : σ.slicing.intervalProp C a₁ b₁ ≤ σ.slicing.intervalProp C a₀ b₀ :=
    σ.slicing.intervalProp_mono C
      ...
      (by
        dsimp [b₀, b₁]
        linarith)
  have hStrict := interval_strictFiniteLength_of_inclusion C hIncl (fun Y =>
    Slicing.IntervalCat.isStrictFiniteLength_of_isFiniteLength C (hWide t Y)) E
  exact Slicing.IntervalCat.isFiniteLength_of_isStrictFiniteLength C
    σ.slicing hStrict

/-- A local `ε₀ < 1 / 16` wide-sector witness for deformation. -/
theorem StabilityCondition.WithClassMap.exists_epsilon0_sixteenth
    (σ : StabilityCondition.WithClassMap C v) :
...
theorem StabilityCondition.WithClassMap.deformation
    ...
    ∃ ε₀ : ℝ, 0 < ε₀ ∧ ∀ (W : Λ →+ ℂ) (ε : ℝ), 0 < ε → ε < ε₀ →
      stabilitySeminorm C σ (W - σ.Z) < ENNReal.ofReal (Real.sin (Real.pi * ε)) →
      ∃ (τ : StabilityCondition.WithClassMap C v), τ.Z = W ∧
        slicingDist C σ.slicing τ.slicing < ENNReal.ofReal ε := by
  obtain ⟨ε₀, hε₀, hε₀16, hWide⟩ := σ.exists_epsilon0_sixteenth C
  refine ⟨ε₀ / 2, by positivity, fun W ε hε hεε₀ hsin => ?_⟩
  obtain ⟨τ, hτZ, hτmem⟩ :=
    σ.exists_eq_Z_and_mem_basisNhd_of_stabilitySeminorm_lt_sin C W
      (lt_of_lt_of_le hsin (ENNReal.ofReal_le_ofReal (Real.sin_le_one _)))
      ε₀ hε₀ hε₀16 hWide ε hε hεε₀ hsin
  exact ⟨τ, hτZ, hτmem.2⟩
...
theorem linearInterpolationZ_sub_sub (σ τ : StabilityCondition.WithClassMap C v) (s t : ℝ) :
  ...
  ext x
  change
    σ.Z x + (t : ℂ) * (τ.Z x - σ.Z x) -
        (σ.Z x + (s : ℂ) * (τ.Z x - σ.Z x)) =
      ((t - s : ℝ) : ℂ) * (τ.Z x - σ.Z x)
  push_cast
  ring

omit [IsTriangulated C] in
theorem stabilitySeminorm_smul (σ : StabilityCondition.WithClassMap C v) (U : Λ →+ ℂ) (t : ℝ) :
    stabilitySeminorm C σ (t • U) = ENNReal.ofReal |t| * stabilitySeminorm C σ U := by
...
omit [IsTriangulated C] in
theorem stabilitySeminorm_smul_complex (σ : StabilityCondition.WithClassMap C v)
    (U : Λ →+ ℂ) (t : ℂ) :
    stabilitySeminorm C σ (t • U) = ENNReal.ofReal ‖t‖ * stabilitySeminorm C σ U := by
  unfold stabilitySeminorm
  rw [ENNReal.mul_iSup]
...
omit [IsTriangulated C] in
theorem stabilitySeminorm_dominated_of_basisNhd (σ τ : StabilityCondition.WithClassMap C v)
    {ε : ℝ} (hε : 0 < ε) (hε8 : ε < 1 / 8) (hτ : τ ∈ basisNhd C σ ε) :
    ∃ K : ENNReal, K ≠ ⊤ ∧
      ∀ (U : Λ →+ ℂ), stabilitySeminorm C σ U ≤ K * stabilitySeminorm C τ U := by
  ...
  have hZbound : stabilitySeminorm C τ (τ.Z - σ.Z) ≤
      ENNReal.ofReal (M_Z / (c - M_Z)) :=
    stabilitySeminorm_le_of_near C σ τ hε hε2 hd hZdiff (τ.Z - σ.Z) hZdiff_ne
  have hsin_pos : 0 < Real.sin (Real.pi * ε) :=
  ...
  have hbound_lt_cos : M_Z / (c - M_Z) < c := by
    rw [div_lt_iff₀ hcMZ]
    linarith
  have hZτ_bound : stabilitySeminorm C τ (σ.Z - τ.Z) <
      ENNReal.ofReal (Real.cos (Real.pi * ε)) := by
    have : stabilitySeminorm C τ (σ.Z - τ.Z) = stabilitySeminorm C τ (τ.Z - σ.Z) := by
      simp only [stabilitySeminorm, AddMonoidHom.sub_apply]
      congr 1
      ext E
      congr 1
      ext φ
  ...
  have hZτ_ne : stabilitySeminorm C τ (σ.Z - τ.Z) ≠ ⊤ :=
    ne_top_of_lt (lt_trans hZτ_bound ENNReal.ofReal_lt_top)
  set N_Z := (stabilitySeminorm C τ (σ.Z - τ.Z)).toReal with hNZ_def
  ...
  intro U
  by_cases hU : stabilitySeminorm C τ U = ⊤
  · have hK0 : ENNReal.ofReal ((c - N_Z)⁻¹) ≠ 0 :=
    ...
    exact le_top
  · have hbound := stabilitySeminorm_le_of_near C τ σ hε hε2 (by rwa [slicingDist_symm])
        hZτ_bound U hU
    have hbound' :
        stabilitySeminorm C σ U ≤
          ENNReal.ofReal ((stabilitySeminorm C τ U).toReal / (c - N_Z)) := by
      simpa [hNZ_def, hc_def] using hbound
    calc stabilitySeminorm C σ U
        ≤ ENNReal.ofReal ((stabilitySeminorm C τ U).toReal / (c - N_Z)) := hbound'
      _ = ENNReal.ofReal (1 / (c - N_Z)) * stabilitySeminorm C τ U := by
...
theorem stabilitySeminorm_center_dominates_of_basisNhd (σ τ : StabilityCondition.WithClassMap C v)
    {ε : ℝ} (hε : 0 < ε) (hε8 : ε < 1 / 8) (hτ : τ ∈ basisNhd C σ ε) :
    ∃ K : ENNReal, K ≠ ⊤ ∧
      ∀ (U : Λ →+ ℂ), stabilitySeminorm C τ U ≤ K * stabilitySeminorm C σ U := by
  rcases hτ with ⟨hZnorm, hd⟩
  have hε2 : ε < 1 / 2 := by linarith
  have hsin_lt_cos : Real.sin (Real.pi * ε) < Real.cos (Real.pi * ε) := by
    rw [← Real.cos_pi_div_two_sub]
  ...
  intro U
  by_cases hU : stabilitySeminorm C σ U = ⊤
  · have hK0 : ENNReal.ofReal ((c - M_Z)⁻¹) ≠ 0 :=
    ...
    rw [hU, ENNReal.mul_top hK0']
    exact le_top
  · have hbound := stabilitySeminorm_le_of_near C σ τ hε hε2 hd hZdiff U hU
    have hbound' :
        stabilitySeminorm C τ U ≤
          ENNReal.ofReal ((stabilitySeminorm C σ U).toReal / (c - M_Z)) := by
      simpa [hMZ_def, hc_def] using hbound
    calc stabilitySeminorm C τ U
        ≤ ENNReal.ofReal ((stabilitySeminorm C σ U).toReal / (c - M_Z)) := hbound'
      _ = ENNReal.ofReal (1 / (c - M_Z)) * stabilitySeminorm C σ U := by
          rw [div_eq_mul_inv, ENNReal.ofReal_mul ENNReal.toReal_nonneg, ENNReal.ofReal_toReal hU]
          simp [one_div, mul_comm]

omit [IsTriangulated C] in
/-- A basis neighborhood contains its center. -/
theorem basisNhd_self (σ : StabilityCondition.WithClassMap C v) {ε : ℝ}
    (hε : 0 < ε) (hε8 : ε < 1 / 8) :
    σ ∈ basisNhd C σ ε := by
  constructor
...
theorem exists_basisNhd_subset_basisNhd (σ τ : StabilityCondition.WithClassMap C v) {ε : ℝ}
  ...
  obtain ⟨K, hK, hdom⟩ :=
    stabilitySeminorm_dominated_of_basisNhd C σ τ hε hε8 hτ_mem
  have hsinε_pos : 0 < Real.sin (Real.pi * ε) := by
    apply Real.sin_pos_of_pos_of_lt_pi
    · positivity
    · nlinarith [Real.pi_pos]
  have hτfin : stabilitySeminorm C σ (τ.Z - σ.Z) ≠ ⊤ := ne_top_of_lt hτZ
  have hK_eq : ENNReal.ofReal K.toReal = K := ENNReal.ofReal_toReal hK
  set dZ := (stabilitySeminorm C σ (τ.Z - σ.Z)).toReal
  have hdZ_eq : ENNReal.ofReal dZ = stabilitySeminorm C σ (τ.Z - σ.Z) :=
  ...
  constructor
  · have hsub : stabilitySeminorm C σ ((τ'.Z - τ.Z) + (τ.Z - σ.Z)) ≤
      stabilitySeminorm C σ (τ'.Z - τ.Z) + stabilitySeminorm C σ (τ.Z - σ.Z) :=
      stabilitySeminorm_add_le C σ (τ'.Z - τ.Z) (τ.Z - σ.Z)
    have hbound : stabilitySeminorm C σ (τ'.Z - σ.Z) ≤
      ...
      calc stabilitySeminorm C σ (τ'.Z - σ.Z)
          = stabilitySeminorm C σ ((τ'.Z - τ.Z) + (τ.Z - σ.Z)) := by rw [hdecomp]
        _ ≤ stabilitySeminorm C σ (τ'.Z - τ.Z) + stabilitySeminorm C σ (τ.Z - σ.Z) := hsub
        _ ≤ K * ENNReal.ofReal (Real.sin (Real.pi * δ)) +
            stabilitySeminorm C σ (τ.Z - σ.Z) := by
            gcongr
            exact (hdom _).trans (by gcongr)
    have hlt : K * ENNReal.ofReal (Real.sin (Real.pi * δ)) +
        stabilitySeminorm C σ (τ.Z - σ.Z) < ENNReal.ofReal (Real.sin (Real.pi * ε)) := by
...
theorem StabilityCondition.WithClassMap.eq_of_same_Z_near
...
theorem StabilityCondition.WithClassMap.eq_of_same_Z_of_mem_basisNhd
    (σ : StabilityCondition.WithClassMap C v)
...
theorem basisNhd_subset_connectedComponent_small (σ : StabilityCondition.WithClassMap C v)
  ...
  have hWt :
      ∀ t : unitInterval,
        stabilitySeminorm C σ (W t - σ.Z) < ENNReal.ofReal (Real.sin (Real.pi * ε)) := by
    intro t
    ...
    calc stabilitySeminorm C σ (W t - σ.Z)
      = ENNReal.ofReal |(t : ℝ)| * stabilitySeminorm C σ (τ.Z - σ.Z) := by
          simp [W, linearInterpolationZ_sub, stabilitySeminorm_smul]
    _ ≤ ENNReal.ofReal 1 * stabilitySeminorm C σ (τ.Z - σ.Z) :=
          mul_le_mul' hcoef le_rfl
    _ = stabilitySeminorm C σ (τ.Z - σ.Z) := by simp
  ...
  have hW1 :
      ∀ t : unitInterval, stabilitySeminorm C σ (W t - σ.Z) < ENNReal.ofReal 1 := fun t =>
    lt_trans (hWt t) ((ENNReal.ofReal_lt_ofReal_iff zero_lt_one).2 hsinε_lt_one)
  have hγ_exists :
      ∀ t : unitInterval, ∃ ρ : StabilityCondition.WithClassMap C v,
        ρ.Z = W t ∧ ρ ∈ basisNhd C σ ε :=
    fun t ↦ σ.exists_eq_Z_and_mem_basisNhd_of_stabilitySeminorm_lt_sin C
      (W t) (hW1 t) ε₀ hε₀ hε₀16 hWide ε hε hεε₀ (hWt t)
  let γ : unitInterval → StabilityCondition.WithClassMap C v :=
    fun t ↦ Classical.choose (hγ_exists t)
    ...
    · have hsum :
        ...
        calc slicingDist C (γ 1).slicing τ.slicing
          ...
          _ = ENNReal.ofReal (ε + ε) := by
              rw [← ENNReal.ofReal_add (le_of_lt hε) (le_of_lt hε)]
      exact lt_trans hsum <|
        (ENNReal.ofReal_lt_ofReal_iff zero_lt_one).2 (by linarith [hεε₀, hε₀16])
  have hγcont : Continuous γ := by
    ...
    obtain ⟨ε₁, hε₁, hε₁16, hWide₁⟩ := ρ₀.exists_epsilon0_sixteenth C
    let δ : ℝ := min (δ₁ / 2) (ε₁ / 4)
    have hδ : 0 < δ := by
      dsimp [δ]
      refine lt_min ?_ ?_
    ...
    have hδ_lt_δ₁ : δ < δ₁ := by
      dsimp [δ]
      linarith [min_le_left (δ₁ / 2) (ε₁ / 4)]
    have hδ_lt_ε₁ : δ < ε₁ / 2 := by
      dsimp [δ]
      linarith [min_le_right (δ₁ / 2) (ε₁ / 4)]
    have hδ8 : δ < 1 / 8 := by
    ...
    obtain ⟨K, hK, hdom⟩ :=
      stabilitySeminorm_center_dominates_of_basisNhd C σ ρ₀ hε hε8 hρ₀mem
    have hUfin : stabilitySeminorm C ρ₀ (τ.Z - σ.Z) ≠ ⊤ := by
    ...
    set L := (stabilitySeminorm C ρ₀ (τ.Z - σ.Z)).toReal
    have hL_eq : ENNReal.ofReal L = stabilitySeminorm C ρ₀ (τ.Z - σ.Z) :=
    ...
    · intro s hs
      ...
      have hWclose :
          stabilitySeminorm C ρ₀ (W s - ρ₀.Z) < ENNReal.ofReal (Real.sin (Real.pi * δ)) := by
        have hmul := abs_mul_lt_sin_of_lt_div hL_nonneg hsinδ_pos hsη'
        calc stabilitySeminorm C ρ₀ (W s - ρ₀.Z)
            = ENNReal.ofReal (|(s : ℝ) - t|) * stabilitySeminorm C ρ₀ (τ.Z - σ.Z) := by
                dsimp [ρ₀]
                rw [hγZ t]
                rw [show W s - W t = ((s : ℝ) - t) • (τ.Z - σ.Z) by
                  simpa [W] using linearInterpolationZ_sub_sub C σ τ t s]
                rw [stabilitySeminorm_smul]
      ...
      have hWclose1 : stabilitySeminorm C ρ₀ (W s - ρ₀.Z) < ENNReal.ofReal 1 :=
        lt_trans hWclose ((ENNReal.ofReal_lt_ofReal_iff zero_lt_one).2
          (sin_pi_mul_lt_one hδ hδ8))
      obtain ⟨ρ, hρZ, hρmem⟩ :=
        ρ₀.exists_eq_Z_and_mem_basisNhd_of_stabilitySeminorm_lt_sin C (W s) hWclose1
          ε₁ hε₁ hε₁16 hWide₁ δ hδ hδ_lt_ε₁ hWclose
      have hγs_eq_ρ : γ s = ρ := by
        ...
        · have hdist₁ :
          ...
          exact lt_trans hdist₂ <|
            (ENNReal.ofReal_lt_ofReal_iff zero_lt_one).2 (by linarith [hεε₀, hε₀16, hδ8])
...
theorem exists_local_lift_sameComponent_in_basisNhd (σ τ ρ₀ : StabilityCondition.WithClassMap C v)
  ...
  have hsη' : |s - t| < Real.sin (Real.pi * δ) / (2 * (L + 1)) :=
    lt_of_lt_of_le hsη <| by
      ...
      exact min_le_right _ _
  have hWclose :
      stabilitySeminorm C ρ₀ (linearInterpolationZ C σ τ s - ρ₀.Z) <
        ENNReal.ofReal (Real.sin (Real.pi * δ)) := by
    have hmul := abs_mul_lt_sin_of_lt_div hL_nonneg hsinδ_pos hsη'
    calc stabilitySeminorm C ρ₀ (linearInterpolationZ C σ τ s - ρ₀.Z)
        = ENNReal.ofReal |s - t| * stabilitySeminorm C ρ₀ (τ.Z - σ.Z) := by
            rw [hρ₀Z]
        ...
        _ < ENNReal.ofReal (Real.sin (Real.pi * δ)) :=
            (ENNReal.ofReal_lt_ofReal_iff hsinδ_pos).2 hmul
  have hWclose1 :
      stabilitySeminorm C ρ₀ (linearInterpolationZ C σ τ s - ρ₀.Z) < ENNReal.ofReal 1 :=
    lt_trans hWclose ((ENNReal.ofReal_lt_ofReal_iff zero_lt_one).2
      (sin_pi_mul_lt_one hδ hδ8))
  obtain ⟨ρ, hρZ, hρmem⟩ :=
    ρ₀.exists_eq_Z_and_mem_basisNhd_of_stabilitySeminorm_lt_sin C
      (linearInterpolationZ C σ τ s) hWclose1 ε₁ hε₁ hε₁16 hWide₁ δ hδ hδ_lt_ε₁ hWclose
  refine ⟨ρ, hρZ, hsubU hρmem, ?_⟩
  exact basisNhd_subset_connectedComponent_small C ρ₀ hε₁ hε₁16 hWide₁ hδ hδ_lt_ε₁ hρmem
