...
/-- Two positive polar representatives whose phases differ by less than two
have the same phase. -/
theorem eq_of_pos_mul_exp_eq {m₁ m₂ φ ψ : ℝ} (hm₁ : 0 < m₁)
    (hm₂ : 0 < m₂) (habs : |φ - ψ| < 2)
  ...
  have hsmall : |Real.pi * φ - Real.pi * ψ| < 2 * Real.pi := by
    rw [← mul_sub, abs_mul, abs_of_pos hpi]
    nlinarith
  have hn0 : n = 0 := by
    by_contra h
    have h2 : 2 * Real.pi ≤ |(n : ℝ)| * (2 * Real.pi) := by
      exact le_mul_of_one_le_left (by positivity)
        (by exact_mod_cast Int.one_le_abs h)
    have h3 : |Real.pi * φ - Real.pi * ψ| =
        |(n : ℝ)| * (2 * Real.pi) := by
      have heq' : Real.pi * φ - Real.pi * ψ = -(n • (2 * Real.pi)) := by
        linarith
      rw [heq', abs_neg, zsmul_eq_mul, abs_mul,
...
/-- Equal central charges determine the phase of a common nonzero semistable
object inside a branch of width two. -/
theorem StabilityCondition.WithClassMap.phase_eq_of_same_Z
    (σ τ : StabilityCondition.WithClassMap C v) (hZ : σ.Z = τ.Z)
    {E : C} {φ ψ : ℝ} (hσ : σ.slicing.P φ E)
    (hτ : τ.slicing.P ψ E) (hE : ¬IsZero E)
    (habs : |φ - ψ| < 2) : φ = ψ := by
  obtain ⟨m₁, hm₁, h₁⟩ := σ.compat φ E hσ hE
  obtain ⟨m₂, hm₂, h₂⟩ := τ.compat ψ E hτ hE
  have hcharge : σ.charge E = τ.charge E := by
    unfold PreStabilityCondition.WithClassMap.charge
    rw [hZ]
  rw [hcharge] at h₁
...
theorem StabilityCondition.WithClassMap.P_of_Q_of_P_semistable
  ...
  have hbds := intrinsicPhaseBounds_of_semistable_slicingDist C σ.slicing τ.slicing
    hE hτ zero_lt_one hd
...
theorem StabilityCondition.WithClassMap.phiMinus_le_le_phiPlus
  ...
  simp only [HNFiltration.phiPlus] at hpP
  simp only [HNFiltration.phiMinus] at hpM
  have hbds := intrinsicPhaseBounds_of_semistable_slicingDist C σ.slicing τ.slicing
    hE hτ zero_lt_one hd
...
lemma gt_phases_of_gtProp (s : Slicing C) {X : C} {φ : ℝ}
  ...
  · obtain ⟨FX, hnFX, _, hlast⟩ := s.exists_hn_nonzero_boundaries C hXne
    have h1 : GX.phiMinus C hnX ≤ FX.phiMinus C hnFX :=
      GX.phiMinus_le_of_lastFactor_nonzero C s FX hnX hnFX hlast
    have h2 : FX.phiMinus C hnFX = s.phiMinus C X hXne := by
      exact (s.phiMinus_eq C X hXne FX hnFX hlast).symm
    linarith
...
lemma phiPlus_le_of_leProp (s : Slicing C) {Y : C} {φ : ℝ}
  ...
  · obtain ⟨FY, hnFY, hfirst, _⟩ := s.exists_hn_nonzero_boundaries C hYne
    have h1 : FY.phiPlus C hnFY ≤ GY.phiPlus C hnY :=
      FY.phiPlus_le_of_firstFactor_nonzero C s GY hnFY hnY hfirst
    have h2 : s.phiPlus C Y hYne = FY.phiPlus C hnFY := by
      exact s.phiPlus_eq C Y hYne FY hnFY hfirst
    linarith
...
private theorem bridgeland_6_4_one_dir
  ...
  simp only [HNFiltration.phiPlus] at hF0
  simp only [HNFiltration.phiMinus] at hFn
  ...
  push Not at hall_eq
  -- Step 2: Split E at the cutoff while retaining the inherited upper bound.
  obtain ⟨X, Y, f, g, h, hT, hXgt, hYle, hXub⟩ :=
    σ.slicing.exists_split_at_cutoff_with_upper_bound C F hF_interval hn
  ...
  suffices hXZ : IsZero X by
    ...
    · exact σ.slicing.zero_mem_of_isZero C φ Y hYZ
    ...
    simp only [HNFiltration.phiPlus] at hFY0
    simp only [HNFiltration.phiMinus] at hFYn
    ...
    have hYσM : φ - 1 < σ.slicing.phiMinus C Y hYZ := by
      ...
      have hbdsY := intrinsicPhaseBounds_of_semistable_slicingDist C
        σ.slicing τ.slicing hYZ hτY zero_lt_one hd
    ...
    obtain ⟨FY', hnY', hfirst', hlast'⟩ := σ.slicing.exists_hn_nonzero_boundaries C hYZ
    have hm_eq := σ.slicing.phiMinus_eq C Y hYZ FY' hnY' hlast'
    have hm_eq' : σ.slicing.phiMinus C Y hYZ =
        FY'.φ ⟨FY'.n - 1, by lia⟩ := by
      simpa only [HNFiltration.phiMinus] using hm_eq
  ...
  obtain ⟨GX, hnGX, hGXfirst, hGXlast⟩ := τ.slicing.exists_hn_nonzero_boundaries C hXne
  -- Show τ.phiPlus(X) ≤ φ
    ...
    have hU_maps_zero : ∀ α : U ⟶ X, α = 0 := by
      ...
      · -- Use canonical σ-HN filtrations for both U and Y⟦-1⟧
        obtain ⟨FU, hnFU, hFUfirst, hFUlast⟩ :=
          σ.slicing.exists_hn_nonzero_boundaries C hGXfirst
        obtain ⟨FYs, hnFYs, hFYsfirst, hFYslast⟩ :=
          σ.slicing.exists_hn_nonzero_boundaries C hYsZ
        -- σ-phases of U: min > ψ - 1
        have hU_σ_bds :=
          intrinsicPhaseBounds_of_semistable_slicingDist C σ.slicing τ.slicing
            hGXfirst (GX.semistable ⟨0, hnGX⟩) zero_lt_one hd
        have hU_σ_M : ψ - 1 < FU.φ ⟨FU.n - 1, by lia⟩ := by
          have heq := σ.slicing.phiMinus_eq C U hGXfirst FU hnFU hFUlast
          simp only [HNFiltration.phiMinus] at heq
          rw [← heq]
          exact (Set.mem_Ioo.mp hU_σ_bds.2).1
        -- σ-phases of Y⟦-1⟧: max ≤ φ - 1
        have hYs_le : σ.slicing.leProp C (φ - 1) (Y⟦(-1 : ℤ)⟧) := by
          simpa only [Int.cast_neg, Int.cast_one, sub_eq_add_neg] using
            σ.slicing.leProp_shift C φ Y (-1) hYle
        have hYs_P : FYs.φ ⟨0, hnFYs⟩ ≤ φ - 1 := by
          have heq := σ.slicing.phiPlus_eq C _ hYsZ FYs hnFYs hFYsfirst
          simp only [HNFiltration.phiPlus] at heq
          rw [← heq]
  ...
  have hτM_gt : φ - 1 < τ.slicing.phiMinus C X hXne := by
    have hM_bd := abs_phiMinus_sub_lt_of_slicingDist C σ.slicing τ.slicing
      hXne zero_lt_one hd
    rw [abs_lt] at hM_bd; linarith
  -- Get canonical σ-HN and τ-HN of X with all phase bounds
  obtain ⟨FX, hnFX, hFXfirst, hFXlast⟩ := σ.slicing.exists_hn_nonzero_boundaries C hXne
