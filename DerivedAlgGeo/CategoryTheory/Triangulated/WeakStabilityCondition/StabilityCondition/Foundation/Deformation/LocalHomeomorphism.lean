...
def componentRep (cc : ConnectedComponents (StabilityCondition.WithClassMap C v)) :
    StabilityCondition.WithClassMap C v :=
  Classical.choose cc.exists_rep

omit [IsTriangulated C] in
@[simp] theorem mk_componentRep
...
abbrev componentStabilityCondition
    (cc : ConnectedComponents (StabilityCondition.WithClassMap C v)) :=
  {σ : StabilityCondition.WithClassMap C v // ConnectedComponents.mk σ = cc}

/-- The owner-native local-homeomorphism statement for the central charge on every
connected component of the stability space. -/
def StabilityCondition.WithClassMap.CentralChargeIsLocalHomeomorphOnConnectedComponents : Prop :=
  ∀ (cc : ConnectedComponents (StabilityCondition.WithClassMap C v)),
    ∃ (V : Submodule ℂ (Λ →+ ℂ))
      ...
      @IsLocalHomeomorph
        (componentStabilityCondition C cc)
...
def componentSeminormSubgroup (cc : ConnectedComponents (StabilityCondition.WithClassMap C v)) :
  ...
  smul_mem' t U hU := by
    change stabilitySeminorm C (componentRep C cc) (t • U) < ⊤
    rw [stabilitySeminorm_smul_complex]
...
noncomputable instance componentNorm
  ...
  norm U := (stabilitySeminorm C (componentRep C cc) (U : Λ →+ ℂ)).toReal
...
noncomputable def componentAddGroupNorm
  ...
  toFun U := ‖U‖
  map_zero' := by
    change (stabilitySeminorm C (componentRep C cc) (0 : Λ →+ ℂ)).toReal = 0
    rw [stabilitySeminorm_zero, ENNReal.toReal_zero]
  add_le' U V := by
    change (stabilitySeminorm C (componentRep C cc) (((U + V : componentSeminormSubgroup C cc) :
      Λ →+ ℂ))).toReal ≤
      (stabilitySeminorm C (componentRep C cc) (U : Λ →+ ℂ)).toReal +
        (stabilitySeminorm C (componentRep C cc) (V : Λ →+ ℂ)).toReal
    rw [show (((U + V : componentSeminormSubgroup C cc) : Λ →+ ℂ)) =
      (U : Λ →+ ℂ) + (V : Λ →+ ℂ) by rfl]
    have hle := stabilitySeminorm_add_le C (componentRep C cc) (U : Λ →+ ℂ) (V : Λ →+ ℂ)
    have hle' :
        stabilitySeminorm C (componentRep C cc) (((U + V : componentSeminormSubgroup C cc) :
          Λ →+ ℂ)) ≤
          stabilitySeminorm C (componentRep C cc) (U : Λ →+ ℂ) +
            stabilitySeminorm C (componentRep C cc) (V : Λ →+ ℂ) := by
      simpa using hle
    have htoReal :
        (stabilitySeminorm C (componentRep C cc) (((U + V : componentSeminormSubgroup C cc) :
          Λ →+ ℂ))).toReal ≤
          (stabilitySeminorm C (componentRep C cc) (U : Λ →+ ℂ) +
            stabilitySeminorm C (componentRep C cc) (V : Λ →+ ℂ)).toReal := by
  ...
  neg' U := by
    change (stabilitySeminorm C (componentRep C cc) (-(U : Λ →+ ℂ))).toReal =
      (stabilitySeminorm C (componentRep C cc) (U : Λ →+ ℂ)).toReal
    rw [stabilitySeminorm_neg]
  eq_zero_of_map_eq_zero' U hU := by
    apply Subtype.ext
    change (stabilitySeminorm C (componentRep C cc) (U : Λ →+ ℂ)).toReal = 0 at hU
    exact eq_zero_of_stabilitySeminorm_toReal_eq_zero C (componentRep C cc) Fact.out U.2 hU
...
noncomputable instance componentNormedSpace
  ...
  NormedSpace.ofCore
    { norm_nonneg := fun U ↦ ENNReal.toReal_nonneg
      norm_smul := by
        intro a U
        change (stabilitySeminorm C (componentRep C cc) (((a • U : componentSeminormSubgroup C cc) :
          Λ →+ ℂ))).toReal =
            ‖a‖ * (stabilitySeminorm C (componentRep C cc) (U : Λ →+ ℂ)).toReal
        rw [show (((a • U : componentSeminormSubgroup C cc) : Λ →+ ℂ)) =
            a • (U : Λ →+ ℂ) by rfl]
        rw [stabilitySeminorm_smul_complex, ENNReal.toReal_mul,
...
def componentSeminormBall (cc : ConnectedComponents (StabilityCondition.WithClassMap C v))
  ...
  {F | stabilitySeminorm C (componentRep C cc) (↑F - ↑W) < ENNReal.ofReal r}

omit [IsTriangulated C] in
/-- The old seminorm balls are exactly the metric balls for the induced norm on `V(Σ)`. -/
theorem componentSeminormBall_eq_ball
    (cc : ConnectedComponents (StabilityCondition.WithClassMap C v))
  ...
  change stabilitySeminorm C (componentRep C cc) (↑F - ↑W) < ENNReal.ofReal r ↔
    (stabilitySeminorm C (componentRep C cc) (((F - W : componentSeminormSubgroup C cc) :
      Λ →+ ℂ))).toReal < r
  rw [show (((F - W : componentSeminormSubgroup C cc) : Λ →+ ℂ)) = ↑F - ↑W by rfl]
  have hfin : stabilitySeminorm C (componentRep C cc) (↑F - ↑W) ≠ ⊤ := ne_top_of_lt (F - W).2
...
theorem componentSeminorm_lt_top_of_mem_component
    ...
    stabilitySeminorm C σ (U : Λ →+ ℂ) < ⊤ := by
...
theorem componentNorm_equivalent_of_mem_component
    ...
    ∃ K L : ℝ, 0 < K ∧ 0 < L ∧
      ∀ U : componentSeminormSubgroup C cc,
        ‖U‖ ≤ K * (stabilitySeminorm C σ (U : Λ →+ ℂ)).toReal ∧
        (stabilitySeminorm C σ (U : Λ →+ ℂ)).toReal ≤ L * ‖U‖ := by
  ...
  obtain ⟨A, hA, hdomA⟩ := stabilitySeminorm_dominated_of_connected C σ₀ σ hσ₀σ
  obtain ⟨B, hB, hdomB⟩ := stabilitySeminorm_dominated_of_connected C σ σ₀ hσ₀σ.symm
  refine ⟨A.toReal + 1, B.toReal + 1, by positivity, by positivity, ?_⟩
  intro U
  have hUσ : stabilitySeminorm C σ (U : Λ →+ ℂ) < ⊤ :=
    componentSeminorm_lt_top_of_mem_component C cc σ hσ U
  constructor
  · have hleA :
        ‖U‖ ≤ A.toReal * (stabilitySeminorm C σ (U : Λ →+ ℂ)).toReal := by
      change (stabilitySeminorm C σ₀ (U : Λ →+ ℂ)).toReal ≤
        A.toReal * (stabilitySeminorm C σ (U : Λ →+ ℂ)).toReal
      have hleA' :
          (stabilitySeminorm C σ₀ (U : Λ →+ ℂ)).toReal ≤
            (A * stabilitySeminorm C σ (U : Λ →+ ℂ)).toReal :=
    ...
    have hσ_nonneg : 0 ≤ (stabilitySeminorm C σ (U : Λ →+ ℂ)).toReal := ENNReal.toReal_nonneg
    nlinarith [hleA]
  · have hleB :
        (stabilitySeminorm C σ (U : Λ →+ ℂ)).toReal ≤ B.toReal * ‖U‖ := by
      change (stabilitySeminorm C σ (U : Λ →+ ℂ)).toReal ≤
        B.toReal * (stabilitySeminorm C σ₀ (U : Λ →+ ℂ)).toReal
      have hleB' :
          (stabilitySeminorm C σ (U : Λ →+ ℂ)).toReal ≤
            (B * stabilitySeminorm C σ₀ (U : Λ →+ ℂ)).toReal :=
  ...
  · have hLocal :
      ...
      · refine IsOpen.mem_nhds ?_ ?_
        ...
        · show σ ∈ basisNhd C σ ε
          constructor
          · show stabilitySeminorm C σ (σ.Z - σ.Z) < _
      ...
      · -- Continuity (Prop 6.3 + Lemma 6.2)
        have hZcont : Continuous Zmap := by
          ...
          obtain ⟨K, hK, hdom⟩ := stabilitySeminorm_dominated_of_connected C σ₀ τ' hconn_τ'
          ...
          -- hτ'_mem : stabilitySeminorm C σ₀ (↑(Zmap ⟨τ', hτ'cc⟩) - ↑W) < ENNReal.ofReal r
          change stabilitySeminorm C σ₀ (τ'.Z - ↑W) < ENNReal.ofReal r at hτ'_mem
          -- Finiteness and gap
          have hfin : stabilitySeminorm C σ₀ (τ'.Z - ↑W) ≠ ⊤ := ne_top_of_lt hτ'_mem
          have hK_eq : ENNReal.ofReal K.toReal = K := ENNReal.ofReal_toReal hK
          set d := (stabilitySeminorm C σ₀ (τ'.Z - ↑W)).toReal
          have hd_eq : ENNReal.ofReal d = stabilitySeminorm C σ₀ (τ'.Z - ↑W) :=
          ...
          refine Filter.mem_of_superset
            (IsOpen.mem_nhds
              ...
              (show τ' ∈ basisNhd C τ' δ from
                ⟨by rw [sub_self, stabilitySeminorm_zero]
          ...
          show stabilitySeminorm C σ₀ (τ''.Z - ↑W) < ENNReal.ofReal r
          -- Connectivity for τ''
          have hconn'' : ConnectedComponents.mk σ₀ = ConnectedComponents.mk τ'' := by
            rw [show σ₀ = componentRep C cc by rfl, mk_componentRep C cc, hτ''cc]
          -- Subadditivity: ‖A+B‖ ≤ ‖A‖ + ‖B‖ for stabilitySeminorm
          have hsub : stabilitySeminorm C σ₀ ((τ''.Z - τ'.Z) + (τ'.Z - ↑W)) ≤
              stabilitySeminorm C σ₀ (τ''.Z - τ'.Z) + stabilitySeminorm C σ₀ (τ'.Z - ↑W) := by
            apply iSup_le; intro E; apply iSup_le; intro φ
            apply iSup_le; intro hP; apply iSup_le; intro hE
            calc ENNReal.ofReal (‖((τ''.Z - τ'.Z) + (τ'.Z - ↑W)) (classOf C v E)‖ /
                    ‖σ₀.Z (classOf C v E)‖)
                ≤ ENNReal.ofReal (‖(τ''.Z - τ'.Z) (classOf C v E)‖ / ‖σ₀.Z (classOf C v E)‖ +
                    ‖(τ'.Z - ↑W) (classOf C v E)‖ / ‖σ₀.Z (classOf C v E)‖) := by
              ...
              _ = ENNReal.ofReal (‖(τ''.Z - τ'.Z) (classOf C v E)‖ / ‖σ₀.Z (classOf C v E)‖) +
                  ENNReal.ofReal (‖(τ'.Z - ↑W) (classOf C v E)‖ / ‖σ₀.Z (classOf C v E)‖) :=
                ENNReal.ofReal_add (div_nonneg (norm_nonneg _) (norm_nonneg _))
                  (div_nonneg (norm_nonneg _) (norm_nonneg _))
              _ ≤ stabilitySeminorm C σ₀ (τ''.Z - τ'.Z) + stabilitySeminorm C σ₀ (τ'.Z - ↑W) :=
                add_le_add
          ...
          have hbound : stabilitySeminorm C σ₀ (τ''.Z - ↑W) ≤
              K * ENNReal.ofReal (Real.sin (Real.pi * δ)) +
                stabilitySeminorm C σ₀ (τ'.Z - ↑W) := by
            have hdecomp : (τ''.Z - ↑W : Λ →+ ℂ) = (τ''.Z - τ'.Z) + (τ'.Z - ↑W) := by
              ext; simp [AddMonoidHom.sub_apply, sub_add_sub_cancel]
            calc stabilitySeminorm C σ₀ (τ''.Z - ↑W)
                = stabilitySeminorm C σ₀ ((τ''.Z - τ'.Z) + (τ'.Z - ↑W)) := by rw [hdecomp]
              _ ≤ stabilitySeminorm C σ₀ (τ''.Z - τ'.Z) + stabilitySeminorm C σ₀ (τ'.Z - ↑W) := hsub
          ...
          have hlt : K * ENNReal.ofReal (Real.sin (Real.pi * δ)) +
              stabilitySeminorm C σ₀ (τ'.Z - ↑W) < ENNReal.ofReal r := by
        ...
        have hZopen : IsOpenMap (U.restrict Zmap) := by
          ...
          obtain ⟨K_rev, hK_rev, hdom_rev⟩ :=
            stabilitySeminorm_dominated_of_connected C σ_x σ₀ hconn_x.symm
          ...
          obtain ⟨ε₀_x, hε₀_x, hε₀_x16, hWide_x⟩ := σ_x.exists_epsilon0_sixteenth C
          let δ : ℝ := min (ε₀_x / 4) (min δT δU)
          have hδ : 0 < δ := by
            dsimp [δ]
            refine lt_min ?_ ?_
            · linarith
          ...
          have hδ_lt_ε₀x : δ < ε₀_x / 2 := by
            dsimp [δ]
            linarith [min_le_left (ε₀_x / 4) (min δT δU)]
          have hδ8 : δ < 1 / 8 :=
          ...
          let B : Set V := {F : V | stabilitySeminorm C σ₀ (↑F - σ_x.Z) < ENNReal.ofReal r0}
          refine Filter.mem_of_superset
            (IsOpen.mem_nhds
              (by
                ...
                exact TopologicalSpace.GenerateOpen.basic _ ⟨Zmap ⟨σ_x, hσ_x_cc⟩, r0, hr0, by
                  ...
                  change componentSeminormBall C cc (Zmap ⟨σ_x, hσ_x_cc⟩) r0 F ↔
                    stabilitySeminorm C σ₀ (↑F - σ_x.Z) < ENNReal.ofReal r0
                  change
                    stabilitySeminorm C (componentRep C cc)
                        (↑F - ↑(componentZMap C cc ⟨σ_x, hσ_x_cc⟩)) <
                      ENNReal.ofReal r0 ↔
                      stabilitySeminorm C (componentRep C cc) (↑F - σ_x.Z) <
                        ENNReal.ofReal r0
                  rfl⟩)
              (by
                change stabilitySeminorm C σ₀ (σ_x.Z - σ_x.Z) < ENNReal.ofReal r0
                rw [sub_self, stabilitySeminorm_zero]
                exact ENNReal.ofReal_pos.mpr hr0))
            ?_
          intro F hF
          have hFσ₀ : stabilitySeminorm C σ₀ ((F : V) - σ_x.Z) < ENNReal.ofReal r0 := by
            simpa [B] using hF
          have hFfin : stabilitySeminorm C σ₀ ((F : V) - σ_x.Z) ≠ ⊤ := ne_top_of_lt hFσ₀
          have hK_eq : ENNReal.ofReal K_rev.toReal = K_rev := ENNReal.ofReal_toReal hK_rev
          set d := (stabilitySeminorm C σ₀ ((F : V) - σ_x.Z)).toReal
          have hd_eq : ENNReal.ofReal d = stabilitySeminorm C σ₀ ((F : V) - σ_x.Z) :=
            ENNReal.ofReal_toReal hFfin
          have hd_lt : d < r0 := by
            rwa [← hd_eq, ENNReal.ofReal_lt_ofReal_iff hr0] at hFσ₀
          have hWclose :
              stabilitySeminorm C σ_x ((F : V) - σ_x.Z) <
            ...
            calc
              stabilitySeminorm C σ_x ((F : V) - σ_x.Z)
                  ≤ K_rev * stabilitySeminorm C σ₀ ((F : V) - σ_x.Z) := hdom_rev _
          ...
          have hWclose1 : stabilitySeminorm C σ_x ((F : V) - σ_x.Z) < ENNReal.ofReal 1 :=
            lt_trans hWclose
              ((ENNReal.ofReal_lt_ofReal_iff zero_lt_one).2 hsinδ_lt_one)
          obtain ⟨ρ, hρZ, hρmem⟩ :=
            σ_x.exists_eq_Z_and_mem_basisNhd_of_stabilitySeminorm_lt_sin C (F : V) hWclose1
              ε₀_x hε₀_x hε₀_x16 hWide_x δ hδ hδ_lt_ε₀x hWclose
          have hρccx :
              ConnectedComponents.mk ρ = ConnectedComponents.mk σ_x :=
            basisNhd_subset_connectedComponent_small C σ_x hε₀_x hε₀_x16 hWide_x hδ hδ_lt_ε₀x hρmem
