...
/-!
# Fourier--Mukai kernel autoequivalences

This is the categorical source of truth for a kernel functor that is an
autoequivalence. It contains the presentation by a correspondence and kernel,
dual and unit kernels, convolution of kernel autoequivalences, and the induced
automorphism of `K₀`.
...
* **Nothing constructs a `DualKernel` from geometry.** That the quasi-inverse
  ...
  file proves the categorical consequences of having one available, including
  that the two kernels' class maps are mutually inverse
  (`transformK₀_dual_comp`).
...
* **No group of kernel autoequivalences.** Composing two kernel
  autoequivalences needs a `ConvolutionData` supplied per pair (`trans`), and
  the unit needs a supplied `UnitKernelData`. No identity law relating those
...
* Nothing about stability conditions, Bridgeland's `Stab(X)` as a manifold,
  or continuity of an induced action.
-/

universe w u x t x₁ x₂ x₃ t₁ t₂ t₃
