...
`CentralCharge/Family.lean` defines parameterized additive charges.  This file
supplies a divisorial descendant without importing wall equations.
...
Wall loci and their determinant formulas are added downstream in
`Walls/Divisorial/Slice.lean`.  Ampleness and the Hodge-index signature are
separate proposition-valued certificates; neither is required to define the
charge family.
-/
