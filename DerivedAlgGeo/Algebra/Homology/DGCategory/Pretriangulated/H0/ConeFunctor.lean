...
Everything here is intrinsic to the dg category: no ordinary category has been
chosen, and no comparison equivalence appears. Reading these cone triangles in
an enhanced category is the adapter
`CategoryTheory/Triangulated/DGEnhancement/H0/ConeFunctor.lean`, which is where
the shift and exactness compatibilities of a comparison are required.
-/
