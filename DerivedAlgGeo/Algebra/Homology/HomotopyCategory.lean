...
`CohomologyClass` degree-zero consequences, and a map-natural linear class
adapter are also owned here. The enhancement *interface* is a structure on an
abstract triangulated category and lives in
